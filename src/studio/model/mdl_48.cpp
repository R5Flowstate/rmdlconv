...
//
// ConvertStudioHdr
// Purpose: converts the mdl v48 (Garry's Mod) studiohdr_t struct to rmdl v8 compatible (Apex Legends Season 0-6)
void ConvertStudioHdrFrom48To54(r5::v8::studiohdr_t* out, studiohdr_t* hdr)
{
	out->id = 'TSDI';
	out->version = 54;
...
void ConvertBones_48(mstudiobone_t* pOldBones, int numBones, bool isRig)
{
	...
	for (int i = 0; i < numBones; ++i)
	{
		...
		newBone->scale = Vector{ 1.f, 1.f, 1.f };
...
void ConvertHitboxes_48(mstudiohitboxset_t* pOldHitboxSets, int numHitboxSets)
{
...
void ConvertBodyParts_48(mstudiobodyparts_t* pOldBodyParts, int numBodyParts)
{
...
void ConvertIkChains_48(mstudioikchain_t* pOldIkChains, int numIkChains, bool isRig)
{
...
void ConvertTextures_48(mstudiotexturedir_t* pCDTextures, int numCDTextures, mstudiotexture_t* pOldTextures, int numTextures)
{
...
void ConvertSkins_48(char* pOldSkinData, int numSkinRef, int numSkinFamilies)
{
	// TODO[rexx]: maybe add old cdtexture parsing here if available, or give the user the option to manually set the material paths
...
void ConvertMDL48To54(char* pMDL, const std::string& pathIn, const std::string& pathOut)
{
	...
	printf("Converting model '%s' from version 48 to version 54 (subversion 10)...\n", rawModelName.c_str());
	...
	ConvertStudioHdrFrom48To54(pHdr, oldHeader);
	g_model.pHdr = pHdr;
	...
	ConvertBones_48((mstudiobone_t*)input.getPtr(), oldHeader->numbones, false);
	...
	ConvertHitboxes_48((mstudiohitboxset_t*)input.getPtr(), oldHeader->numhitboxsets);
	...
	ConvertBodyParts_48((mstudiobodyparts_t*)input.getPtr(), oldHeader->numbodyparts);
	...
	ConvertIkChains_48((mstudioikchain_t*)input.getPtr(), oldHeader->numikchains, false);
	...
	ConvertTextures_48((mstudiotexturedir_t*)pOldCDTextures, oldHeader->numcdtextures, (mstudiotexture_t*)input.getPtr(), oldHeader->numtextures);

	// convert skin data
	input.seek(oldHeader->skinindex, rseekdir::beg);
	ConvertSkins_48((char*)input.getPtr(), oldHeader->numskinref, oldHeader->numskinfamilies);
